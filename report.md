# PCA-KRR 기반 거리 Fingerprint 위치 추정 알고리즘
...
참고한 논문은 RSS fingerprint를 이용한 실내 위치 추정에서 PCA feature reduction과 Kernel Ridge Regression을 결합하였다. 본 프로젝트는 해당 구조를 참고하되, RSS 값이 아니라 18개 기지국의 RTT 기반 거리 추정값을 distance fingerprint로 사용한다는 점이 다르다. 또한 참고 논문에서는 radio map 구성과 fingerprint selection 과정을 포함하지만, 본 구현에서는 제공된 18개 기지국의 거리값 전체를 사용하고, cross-validation을 통해 log transform, PCA whitening, KRR parameter를 선택하는 방식으로 구성하였다.

## 3. Agent AI 활용 방안

본 프로젝트에서는 Agent AI로 ChatGPT를 활용하였다. AI는 알고리즘을 대신 결정하거나 결과를 자동으로 생성하는 용도가 아니라, 본인이 설정한 방향을 구체화하고 구현 과정에서 발생할 수 있는 오류를 점검하는 보조 도구로 사용하였다.

먼저 알고리즘 선정 단계에서는 WLS, KNN fingerprinting, Ridge Regression, Kernel Ridge Regression의 차이를 비교하는 데 활용하였다. 본인은 이전 중간 과제에서 WLS 기반 위치 추정을 사용한 경험이 있었고, 이번 final project에서는 같은 구조를 반복하기보다 distance fingerprint 기반 접근을 적용하고자 하였다. 이 과정에서 AI를 이용하여 각 알고리즘의 장단점, hidden test set에서의 일반화 가능성, baseline으로 비교할 수 있는 방법을 정리하였다.

두 번째로 데이터 구조 분석 단계에서 AI를 활용하였다. 본인은 d_hat이 18 × 700 형태이고, p가 2 × 700 형태라는 점을 확인하였다. 이후 scikit-learn 모델에 입력하기 위해서는 샘플이 행 방향이 되도록 N × 18, N × 2 형태로 변환해야 한다는 점을 점검하였다. AI는 이 과정에서 shape error가 발생하지 않도록 입력과 출력 형태를 확인하는 데 도움을 주었다.

세 번째로 구현 단계에서는 train.py와 main.py의 역할을 분리하는 방향을 점검하였다. train.py는 제공된 train data를 이용해 모델을 학습하고 validation 성능을 확인한 뒤 model.pkl을 저장하는 역할을 한다. 반면 main.py는 hidden test data가 들어왔을 때 저장된 모델을 불러와 예측 결과만 반환해야 한다. AI는 이 구조가 채점 방식에 맞는지, 출력 p_hat의 shape이 2 × num_user인지 확인하는 데 사용되었다.

네 번째로 성능 개선 단계에서는 기존 PCA-KRR 구조에 log1p 변환, PCA whitening, gamma와 alpha 탐색 범위 확장, RandomizedSearchCV, P90 기준 scorer를 적용하는 방향을 검토하였다. 본인은 train.py를 실행하여 baseline과 최종 모델의 검증 성능을 확인하고, AI는 결과 해석과 보고서 작성 방향을 정리하는 데 활용하였다.

본인의 역할은 데이터셋 구조 확인, distance fingerprint라는 문제 해석 설정, baseline 선정, 최종 알고리즘 선택, validation 방식 결정, 코드 실행 및 결과 해석이었다. AI의 역할은 알고리즘 설명 정리, 코드 구조 점검, shape 및 파일 구조 관련 오류 확인, 보고서 문장 구성 보조였다. 따라서 본 프로젝트에서 AI는 설계와 구현을 보조하는 도구로 사용되었으며, 최종 판단과 결과 해석은 본인이 수행하였다.

## 4. 결과 도출 & 디스커션

성능 평가는 제공된 700개 데이터를 이용하여 5-Fold cross-validation 방식으로 수행하였다. 전체 데이터를 학습에만 사용하면 모델이 실제로 일반화되는지 확인하기 어렵기 때문에, 동일한 fold split을 사용하여 WLS, KNN, Ridge Regression, PCA-KRR을 비교하였다. 본 프로젝트에서는 평균 위치 오차뿐만 아니라 일부 큰 위치 오차가 발생하는 경우도 중요하다고 판단하여, hyperparameter 선택 기준으로 P90 위치 오차를 사용하였다.
...
Baseline은 WLS, KNN, Ridge Regression으로 설정하였다. WLS는 기지국 좌표와 거리값의 기하학적 관계를 이용하는 물리 기반 baseline이다. KNN은 distance fingerprint의 유사도를 이용하는 fingerprinting baseline이다. Ridge Regression은 18개 거리값과 위치 좌표 사이의 선형 관계를 학습하는 회귀 baseline이다. PCA-KRR은 이들과 비교하여 distance fingerprint의 비선형 관계를 학습하고, log transform, PCA whitening, ridge 정규화를 통해 안정성을 높인 최종 제안 모델이다.

| 모델               | 사용 정보       | 핵심 아이디어                                                              | 5-Fold CV P90 위치 오차 |
| ---------------- | ----------- | -------------------------------------------------------------------- | ------------------: |
| WLS              | 기지국 좌표, 거리값 | 거리 원의 기하학적 관계를 이용하여 위치 추정                                            |           22.8088 m |
| KNN              | 거리값         | 유사한 거리 fingerprint를 가진 학습 샘플의 위치 이용                                  |           18.8511 m |
| Ridge Regression | 거리값         | 18개 거리값과 x, y 좌표 사이의 선형 관계 학습                                        |           19.3406 m |
| PCA-KRR          | 거리값         | log 변환, standardization, PCA whitening, RBF kernel 회귀를 이용한 비선형 위치 예측 |           13.0973 m |

실험 결과 PCA-KRR 모델의 5-Fold CV P90 위치 오차는 13.0973 m로 나타났다. 이는 WLS의 22.8088 m, KNN의 18.8511 m, Ridge Regression의 19.3406 m보다 낮은 값이다. 따라서 본 데이터셋에서는 개별 거리값의 기하학적 관계만 이용하는 WLS보다, 18개 기지국 거리값 전체를 distance fingerprint로 해석하고 비선형 회귀 모델로 학습하는 방식이 더 적합하다고 판단하였다.

Ridge Regression보다 PCA-KRR의 성능이 좋게 나타난 것은 거리 fingerprint와 실제 위치 좌표 사이의 관계가 단순 선형 관계만으로는 충분히 설명되기 어렵다는 점을 보여준다. KNN 역시 거리 패턴의 유사도를 활용한다는 점에서 fingerprinting 방식에 해당하지만, 최종 PCA-KRR보다 오차가 크게 나타났다. 이는 KNN이 가까운 일부 학습 샘플에 크게 의존하는 반면, KRR은 RBF kernel을 통해 여러 샘플과의 유사도를 종합적으로 반영하기 때문으로 해석할 수 있다.
...
Baseline 비교의 fairness 측면에서는 동일한 5-Fold split을 사용하여 모든 모델을 평가하였다. 또한 KNN, Ridge Regression, PCA-KRR은 모두 동일한 입력 거리값 d_hat을 사용하였다. WLS는 기지국 좌표 BS_positions를 추가로 사용하지만, WLS 자체가 거리 기반 기하학 알고리즘이므로 기지국 좌표를 사용하는 것이 알고리즘의 기본 조건이다. 따라서 본 비교는 각 알고리즘이 위치 추정 문제를 해석하는 방식의 차이를 비교하는 데 적절하다고 판단하였다.
...
| 번호  | 참고문헌                                                                                                                                                                                                                                                             | 본 프로젝트와의 관련성                                                                                                                                                          |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [1] | Yanfen Le, Shijialuo Jin, Hena Zhang, Weibin Shi, and Heng Yao, “Fingerprinting Indoor Positioning Method Based on Kernel Ridge Regression with Feature Reduction,” Wireless Communications and Mobile Computing, vol. 2021, Article ID 6631585, pp. 1–12, 2021. | RSS fingerprint에 PCA feature reduction과 Kernel Ridge Regression을 적용한 실내 위치 추정 연구이다. 본 프로젝트는 이 구조를 참고하되, RSS가 아니라 18개 기지국의 RTT 기반 거리 추정값을 distance fingerprint로 사용하였다. |
| [2] | scikit-learn documentation, “Kernel Ridge Regression.”                                                                                                                                                                                                           | 본 구현에서 사용한 Kernel Ridge Regression의 기본 개념, RBF kernel, alpha parameter 설정을 참고하였다.                                                                                     |
| [3] | scikit-learn documentation, “Principal Component Analysis.”                                                                                                                                                                                                      | 18차원 거리 fingerprint의 PCA 변환, n_components 설정, whitening 적용 방식을 참고하였다.                                                                                                 |
| [4] | scikit-learn documentation, “RandomizedSearchCV.”                                                                                                                                                                                                                | alpha와 gamma를 연속 범위에서 탐색하고, 제한된 시간 안에서 넓은 hyperparameter 공간을 탐색하는 방식에 참고하였다.                                                                                          |
